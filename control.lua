...
-- Subway buildings are entities of type "storage-tank"" internally, because reasons
local BUILDING_TYPE = "storage-tank"


local entrace_type_by_entity_name = {
	["subway-entrace-0"] = {surface_name = "nauvis",  subway_index = 0},
	["subway-entrace-1"] = {surface_name = "subway1", subway_index = 1},
	["subway-entrace-2"] = {surface_name = "subway2", subway_index = 2},
	["subway-entrace-3"] = {surface_name = "subway3", subway_index = 3},
	["subway-entrace-4"] = {surface_name = "subway4", subway_index = 4},
	["subway-entrace-5"] = {surface_name = "subway5", subway_index = 5},
}
...
local function find_factory_by_building(surface, area)
	...
	for _,entity in pairs(candidates) do
		local entrace_type = entrace_type_by_entity_name[entity.name];
		if entrace_type ~= nil then
			return entity
		end
	end
	return nil
...
local function find_entrace_by_position(surface, position)
	local area = {{position.x-0.2, position.y-0.3},{position.x+0.2, position.y}}
	local candidates = surface.find_entities_filtered{area=area, type=BUILDING_TYPE}
	for _,entity in pairs(candidates) do
		local entrace_type = entrace_type_by_entity_name[entity.name];
		if entrace_type ~= nil then
...
local function update_destructible(factory)
	if factory.built and factory.building.valid then
		factory.building.destructible = not settings.global["subway-indestructible-buildings"].value
	end
end

local function subway_surface(n)
	if n < 1 then 
		return game.surfaces["nauvis"];
	end
	local surface_name = "subway" .. n
		...
		if #(game.surfaces) < 256 then
			surface = game.create_surface(surface_name, {
				...
				property_expression_names = {
					cliffiness = "0",
					["tile:factory-floor-1:probability"] = 1000,
					}
				})
			...
			error("Unfortunately you have no available surfaces left for subway. You cannot use subway on this map.")
		end
	end
	return surface
end
...
local function create_factory_position()
	...
	local surface = subway_surface(1)
...
script.on_event({defines.events.on_built_entity, defines.events.on_robot_built_entity, defines.events.script_raised_built, defines.events.script_raised_revive}, function(event)
	...
	local entrace_type = entrace_type_by_entity_name[entity.name];	
	if entrace_type ~= nil then
		local target_surface = subway_surface(entrace_type.subway_index)
		-- CURRENT
		local exit_entity = target_surface.create_entity{
...
local function get_opposite_exit(entity)
	local entrace_type = entrace_type_by_entity_name[entity.name];	
	if entrace_type ~= nil then
		local target_surface = subway_surface(entrace_type.subway_index)
		return find_entrace_by_position(target_surface, entity.position)
	end	
end

script.on_event({defines.events.on_player_mined_entity, defines.events.on_robot_mined_entity}, function(event)
	local entity = event.entity
	local opposit_exit = get_opposite_exit(entity)
	if opposit_exit ~= nil then
		opposit_exit.destroy()
	end
end)
...
local function set_camera(player, factory, inside)
	...
	local ps_preview_size = ps["subway-preview-size"]
	local preview_size = ps_preview_size and ps_preview_size.value or 300
	local ps_preview_zoom = ps["subway-preview-zoom"]
...
local function enter_station(player, entity)
	local entrace_type = entrace_type_by_entity_name[entity.name];
	if entrace_type ~= nil then	
		teleport_player_safely(
			player, subway_surface(entrace_type.subway_index),
...
local function leave_station(player, factory)
	teleport_player_safely(
...
local function player_may_enter_station(player, entity)
	return player.force.name == entity.force.name
			or (player.force.get_friend(entity.force) and settings.global["subway-allied-players-may-enter"].value)
			or settings.global["subway-enemy-players-may-enter"].value
end
		...
		if player.connected and not player.driving and tick - (global.last_player_teleport[player_index] or 0) >= 45 then
			...
			if walking_state.walking then
				...
				or walking_state.direction == defines.direction.northwest then
					-- Enter factory
					local entity = find_factory_by_building(player.surface, {{player.position.x-0.2, player.position.y-0.3},{player.position.x+0.2, player.position.y}})
					if entity ~= nil then
						if math.abs(player.position.x-entity.position.x)<0.6 then
							if player_may_enter_station(player, entity) then
								enter_station(player, entity)
							end
						end
					end
